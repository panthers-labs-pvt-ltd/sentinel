...
public class SecretService {
    ...
    public static String getK8sSecret(String secretName,String namespace,String element)
    {
        String encryptionKey ;
        // Load kubeconfig from default location (~/.kube/config or in-cluster config)
        ApiClient client = null;  // or Config.fromCluster() if running inside a pod
        try {
            ...
            // Get the secret
            V1Secret secret = api.readNamespacedSecret(secretName,
                    namespace, null);

            // Decode base64 encoded encryption key
            assert secret.getData() != null;
            byte[] encryptedKeyBytes = secret.getData().get(element);
            if (encryptedKeyBytes == null) {
                throw new RuntimeException("encryption_key_secret not found in secret!");
            }
            encryptionKey = new String(encryptedKeyBytes, StandardCharsets.UTF_8);
