...
import static org.pantherslabs.chimera.sentinel.datahub.api.mapper.generated.MetadataAspectV2DynamicSqlSupport.aspect;
import static org.pantherslabs.chimera.sentinel.datahub.common.genericUtils.*;


@Service
public class ManageDomainHierarchy {
    List<MetadataChangeProposal> proposals = new ArrayList<>();
    @Autowired
    private GenericMapper genericMapper;

    public void createDomainHierarchy(@Valid Domains domain, String actionType) throws Exception {
        List<MetadataChangeProposal> proposals = new ArrayList<>();
        buildProposalsRecursively(domain, actionType, null, proposals);

        TransactionalDataHubEmitter emitter = new TransactionalDataHubEmitter();
        emitter.emitWithRollback(proposals);
    }
    ...
    private void buildProposalsRecursively(Domains domain, String actionType, String parentName, List<MetadataChangeProposal> proposals) throws Exception {
        ...
        if (domain.getDomainOwners() != null && !domain.getDomainOwners().isEmpty()) {
            ...
            ManageOwnership.createOwners(domainUrns, DOMAIN_ENTITY_NAME, ownersInfo, actionType);
        }
